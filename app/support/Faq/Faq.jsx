...
const Faq = () => {
  ...
  const getSuggestions = (value) => {
    ...
    const filteredSuggestions = inputValue === ''
      ? []
      : startingData.concat(financialData)
          .filter((item) => item.pregunta.toLowerCase().includes(inputValue))
          .slice(0, 4); 
  ...
  const renderSuggestion = (suggestion) => (
    <div className='bg-gray-200 z-[99999] cursor-pointer'>{suggestion.pregunta}</div>
  );
